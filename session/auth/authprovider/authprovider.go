...
func (ap *authProvider) FetchToken(ctx context.Context, req *auth.FetchTokenRequest) (rr *auth.FetchTokenResponse, err error) {
	...
	// respect registry.insecure flag
	client := &http.Client{}
	client.Transport = &http.Transport{TLSClientConfig: &tls.Config{InsecureSkipVerify: req.Insecure}}
	...
	if creds.Secret != "" {
		...
		resp, err := authutil.FetchTokenWithOAuth(ctx, client, nil, "buildkit-client", to)
		if err != nil {
			...
			if errors.As(err, &errStatus) {
				...
				if (errStatus.StatusCode == 405 && to.Username != "") || errStatus.StatusCode == 404 || errStatus.StatusCode == 401 {
					resp, err := authutil.FetchToken(ctx, client, nil, to)
					if err != nil {
						return nil, err
					}
